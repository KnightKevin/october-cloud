package com.simon.ocotber.cloud.gateway.filter;
...
import com.simon.ocotber.cloud.gateway.config.GatewayConfigProperties;
import com.simon.oct.common.core.constant.SecurityConstants;
...
public class PasswordDecoderFilter extends AbstractGatewayFilterFactory {
    ...
    public GatewayFilter apply(Object config) {
        return (exchange, chain) -> {
            ...
            // 不是登录请求直接向下执行
            if (!StrUtil.containsIgnoreCase(request.getURI().getPath(), SecurityConstants.OAUTH_TOKEN_URL)) {
    ...
    private Function decryptAES() {
        return s -> {
            AES aes = new AES(Mode.CFB, Padding.NoPadding,new SecretKeySpec(gatewayConfig.getEncodeKey().getBytes(), KEY_ALGORITHM), new IvParameterSpec(gatewayConfig.getEncodeKey().getBytes()));
            // 获取请求密码并解密
